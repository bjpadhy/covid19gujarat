  ...
  methods: {
    async getData() {
      axios
        .all([
          axios.get(
            "https://twitscrapeapi.herokuapp.com/api/list/bjpadhy/corona-gujarat?count=6"
          ),
          axios.get(
        ...
        .then(
          axios.spread((twitterData, news) => {
            this.tweets = [];
            this.news = [];
            console.log(twitterData);
            console.log(news);
            twitterData.data.forEach(item => {
              let tempObj = {};
              console.log(item);
              tempObj["screenName"] = item.user.name;
              tempObj["text"] = item.text;
              if (item.extended_entities)
                tempObj["image"] =
                  item.extended_entities.media[0].media_url_https;
              tempObj["time"] = item.created_at;
              tempObj["favoriteCount"] = item.favorite_count;
              tempObj["retweetCount"] = item.retweet_count;
